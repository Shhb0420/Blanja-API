...
module.exports = {
  ...
  getAddressByUser: (user_id) => {
    return new Promise((resolve, reject) => {
      const queryString =
        "SELECT a.id_address, a.fullname, a.address, a.city, a.region, a.zip_code, a.country, u.id FROM address_customer AS a JOIN users AS u on u.id = a.id_user WHERE id_user = " +
        user_id;
  ...
  updateAddress: (req, id, user_id) => {
    return new Promise((resolve, reject) => {
      const queryString =
        "UPDATE address_customer SET ? WHERE id_address = " + id;
